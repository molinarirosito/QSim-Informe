...
Por ejemplo, la instrucción \codecent{ADD [AAAA],0x000F} suma el valor 15 al contenido de la celda cuya dirección es \code{AAAA}, y el resultado de ensamblar esta celda es el siguiente código máquina: \codecent{0010001000000000 1010101010101010 0000000000001111}
...
Por ejemplo, si el código máquina de una instrucción es:\codecent{0010001000000000 1010101010101010 0000000000001111} entonces durante el cargado en memoria se ocuparán 3 celdas (de 16 bits) consecutivas.
...
Se eligió el lenguaje Scala para realizar el simulador ya que durante las cursadas de las asignaturas de TPI no se tiene la oportunidad de profundizar en el dominio de este lenguaje ni aprovechar las ventajas que este ofrecía al combinar el manejo de objetos y las características de un lenguaje funcional. Es por ello que se descartó la posibilidad de utilizar un lenguaje mas extendido, como por ejemplo Java.

\item  \textbf{Framework Arena.}
Se utilizó el \textit{framework} Arena para realizar la interfaz de usuario del simulador porque es una herramienta de código abierto que se utiliza en la materia \ui. Al poder ser combinado con \textbf{Scala} nos pareció una buena oportunidad de explotar lo que nos ofrecía para simplificar la definición de la interfaz de usuario, permitiéndonos así enfocarnos en la implementación del modelo. 

\item  \textbf{Eclipse.}
Se eligió utilizar el entorno de programación Eclipse ya que es una herramienta multiplataforma, lo que permitió trabajar en diferentes sistemas operativos. Por otro lado, la comunidad provee plugins para manejar proyectos para Scala.

\item  \textbf{Git}
Se eligió \textit{git} como repositorio externo para trabajar colaborativamente durante el desarrollo, dado que es una herramienta muy extendida en los desarrollos de software libre.
...
\graf[9cm]{ALU}{Diagrama de clase de la \ALU}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\graf[14cm]{BusEntradaSalida_Memoria_CeldasPuertos}{Diagrama de las clases BusEntradaSalida, Memoria, CeldasPuertos y Celda}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\graf[14cm]{CPU}{Diagrama de clase de la CPU}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Intérprete} \label{claseinterprete}
Como se observa en la figura \ref{Interprete} el Interprete es un \ojo{\textit{singletone}}{Poner definición} que tiene la entera responsabilidad de construir un objeto que representa una instrucción determinada a partir de la decodificación de las celdas de memoria que ocupa su código máquina. Se ocupa de la decodificación de la instrucción.
...
Devuelve la representación en string como código fuente, por ejemplo la representación de un objeto
 \textbf{ADD(R0,R7)}, sería: \ojo{\code{ADD R0, R7}}{describir mejor el objeto}. Este mensaje se utiliza en la etapa de desensamblado.
\item  \textbf{código:}
...
La clase W16 que también esta en la figura \ref{ModoDireccionamiento}, representa el dato que es guardado en memoria. Tiene la capacidad de incrementar y decrementar su valor, como también sumarle un entero, devolver su representación binaria y su valor en decimal.\\
...
\graf[4cm]{Instruccion_SinOperandos}{Detalle de clase Instrucción\_SinOperandos}
...
\graf[15cm]{Instruccion_UnOperando}{Diagrama de clase de la Instrucción\_UnOperando}
...
\graf[15cm]{Instruccion_DosOperandos}{Diagrama de clase de la Instrucción\_DosOperandos}
...
\graf[10cm]{Programa}{Diagrama de la clase Programa}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Simulador}

Esta es la clase principal del modelo y la encargada de coordinar la ejecución del programa paso a paso. Como se observa en la figura \ref{simulador} la clase \textbf{Simulador} conoce a una instancia de la clase \textbf{CPU}, a una instancia de la clase \textbf{BusEntradaSalida} y a una instancia de la clase \textbf{Instrucción}, que representa a la instrucción que se esta ejecutando en ese momento.\\
...
\graf[15cm]{simulador}{Diagrama de la clase Simulador}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
